...
export default {
    ...
    methods: {
        async getData() {
            let data = await this.$api.file.folder();
            this.tableData = data
        },
        async handleConfirm() {
            this.dialogFormVisible = false
            if (this.isAdd) {
                await this.$api.file.add(this.$qs.stringify(this.form));
                this.$message.success("数据添加成功")
                this.getData()
            } else {
                await this.$api.file.update(this.$qs.stringify(this.form));
        ...
        async handleEdit({ id }) {
            let data = await this.$api.file.query({ id });
            this.dialogFormVisible = true
            this.isAdd = false
            this.isView = false
            this.isEdit = true
        ...
        async handleDel({ id }) {
            await this.$api.file.delSingle(this.$qs.stringify({ id }));
